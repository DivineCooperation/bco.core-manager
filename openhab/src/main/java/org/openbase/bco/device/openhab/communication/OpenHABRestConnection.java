...
public abstract class OpenHABRestConnection implements Shutdownable {
    ...
    private final GatewayClass findOpenHABGatewayClass() throws CouldNotPerformException {
        for (GatewayClass gatewayClass : Registries.getClassRegistry().getGatewayClasses()) {
    ...
    private String getToken() throws CouldNotPerformException {
        final GatewayClass openHABGatewayClass = findOpenHABGatewayClass();

        for (UnitConfig unitConfig : Registries.getUnitRegistry().getUnitConfigsByUnitType(UnitTemplateType.UnitTemplate.UnitType.GATEWAY)) {
